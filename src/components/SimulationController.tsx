...
export const SimulationController: React.FC<Props> = ({
  ...
  return (
    <div id="simulation-controller" className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs space-y-4">
      {/* Header */}
      <div className="flex items-center justify-between pb-3 border-b border-slate-200">
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            <Play className={`w-3.5 h-3.5 ${isSimulating ? "text-cyan-600 animate-pulse" : ""}`} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
          ...
          return (
            <button
              ...
              className={`text-left p-3 rounded-xl border transition-all ${
                isSelected
                  ? "bg-cyan-50/70 border-cyan-500 shadow-xs ring-1 ring-cyan-400"
            ...
            >
              <div className="flex items-center justify-between mb-1">
                ...
                <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded border ${sc.badgeColor}`}>
                  {sc.badge}
                </span>
              </div>
        ...
        <button
          ...
          className="text-left p-3 rounded-xl border border-rose-300 bg-rose-50/80 hover:bg-rose-100/80 transition-all group flex flex-col justify-between shadow-2xs"
        >
          <div className="flex items-center justify-between mb-1">
            <span className="font-bold text-xs text-rose-800 flex items-center space-x-1">
              <AlertTriangle className="w-3.5 h-3.5 text-rose-600" />
              <span>이상 진동 긴급 주입</span>
            </span>
            <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-rose-200 text-rose-800 border border-rose-300 font-bold">
              Trigger
            </span>
          </div>
