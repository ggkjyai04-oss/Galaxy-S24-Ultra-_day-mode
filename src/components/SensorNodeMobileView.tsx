...
export const SensorNodeMobileView: React.FC<Props> = ({
  ...
  return (
    <div id="sensor-node-mobile-view" className="min-h-screen bg-slate-50 text-slate-900 flex flex-col p-4 max-w-md mx-auto">
      {/* Top Header */}
      <div className="flex items-center justify-between py-3 border-b border-slate-200">
        <button
          onClick={onBackToDashboard}
          className="flex items-center space-x-1.5 text-slate-600 hover:text-slate-900 text-xs font-medium"
        >
          <ArrowLeft className="w-4 h-4" />
          <span>대시보드로 복귀</span>
        </button>
        <span className="text-xs font-mono font-semibold px-2 py-0.5 rounded bg-cyan-50 text-cyan-800 border border-cyan-200">
      ...
      <div className="flex-1 flex flex-col items-center justify-center py-6 text-center space-y-6">
        <div className="relative">
          ...
          >
            <Smartphone className={`w-10 h-10 ${isStreaming ? "text-cyan-600" : "text-slate-400"}`} />
            <span className="text-[11px] font-mono mt-1 text-slate-600 font-medium">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-900">Galaxy S24 Ultra</h2>
        ...
        {/* Live Gauges */}
        {currentSample && (
          <div className="w-full bg-white border border-slate-200 rounded-2xl p-5 space-y-4 shadow-sm">
            <div>
              <div className="text-xs font-mono text-slate-500 mb-1">실시간 진동 가속도 (동적 벡터합)</div>
              <div className="text-4xl font-bold font-mono text-cyan-700">
            ...
            <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-100 text-xs font-mono">
              <div className="bg-slate-50 p-2 rounded-lg border border-slate-200">
                <span className="text-rose-600 block font-bold">X</span>
                <span className="text-slate-800 font-semibold">{currentSample.x.toFixed(2)}</span>
              </div>
              <div className="bg-slate-50 p-2 rounded-lg border border-slate-200">
                <span className="text-emerald-600 block font-bold">Y</span>
                <span className="text-slate-800 font-semibold">{currentSample.y.toFixed(2)}</span>
              </div>
              <div className="bg-slate-50 p-2 rounded-lg border border-slate-200">
                <span className="text-blue-600 block font-bold">Z</span>
                <span className="text-slate-800 font-semibold">{currentSample.z.toFixed(2)}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {lastAnomaly && (
          <div className="w-full p-3.5 bg-rose-50 border border-rose-200 rounded-xl text-left flex items-start space-x-3">
            <AlertOctagon className="w-5 h-5 text-rose-600 flex-shrink-0 mt-0.5 animate-bounce" />
            <div>
              <div className="text-xs font-bold text-rose-800 font-mono">
                이상 진동 감지 (햅틱 경보 발생)
              </div>
        ...
        <div className="w-full space-y-3">
          {!isStreaming ? (
            ...
            <button
              ...
              className="w-full py-3.5 bg-white hover:bg-slate-50 text-rose-600 font-semibold rounded-xl border border-rose-200 flex items-center justify-center space-x-2 text-sm transition-all shadow-2xs"
            >
